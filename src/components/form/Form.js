...
const Form = ({showForm,setShowForm,content,seats,setSeats}) => {
  const [prize,setPrize] = useState(0);
  const [showNext,setShowNext] = useState(false);
  function handleEvent(e){
      e.preventDefault();
      const clickedSeats = JSON.parse(localStorage.getItem("readyToBooked")) || []
      seats.forEach((seat)=>{
        seat.clicked && clickedSeats.push(seat.id);
      })
  ...
  return (
    <form action="" className={prize?"bookForm clicked":"bookForm"} onSubmit={handleEvent}>
       ...
       { showNext && <div className="nextstep">
           ...
           <div> your seats are : 
           {
            (JSON.parse(localStorage.getItem("ReadyToBooked"))).map((id,i)=>{
           ...
           <div className="button" onClick={()=>{
              setSeats(seats.map((seat)=>{
                   if(seat.clicked){
                    return {...seat,booked:true,clicked:false}
                   }else{
                     return seat
                   }
              }))
